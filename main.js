        ...
        alert("HELLO HIHI PRESSS MOUSE");
        var song = new Audio()
        song.src = "https://ia601404.us.archive.org/19/items/christmas-snow-rainy-piano-tiles-2_20211022/Christmas%20Snow%20-%20Rainy%20%E2%94%82Piano%20Tiles%202.mp3";
        
        // set time between two event
        var t = setInterval(function () {
                if (innerWidth && innerHeight) {
                    ...
                    score = 0;
                    speed = 1;
                    asteroids = [];// asteroids is hành tinh nhỏ
        ...
        function draw() {
            ...
            if (start) {
                ...
                ctx.fillText("Rocket 00", c.width/2, c.height/4);
                ctx.beginPath();
                ctx.fillStyle = "white";
                ctx.textBaseline = "middle";
                ctx.textAlign = "center";
                ctx.font = x/12 + "px monospace";
                ctx.fillText(Date.now()%2000<1000?"-Tap to start-hehe":"-Play now-", c.width/2, c.height/2);
            }
    ...
    // end ship
        ...
        class Obstacle {
            ...
            if (this.x < -x) {
                this.passed = false;// dịch chuyển tọa độ tiếp theo
    ...
    // class obstacles success
        ...
        for (var i=2; i < path.length; i += 2) {
            ...
            if ((y - y1) * (y - y2) <= 0 && (x <= x1 || x <= x2) && (x1 >= x && x2 >= x || (x2 - x1) * (y - y1) / (y2 - y1) >= x - x1)) 
            {
